...
selected_genes = []
gene_trends = {}  # mappa gene → 'increasing' o 'decreasing'
for gene in gene_columns:
    ...
    if increasing != decreasing:
        ...
        gene_trends[gene] = "decreasing" if decreasing else "increasing"
...
for gene in selected_genes:
    ...
    trend = gene_trends[gene]
    results_with_pvalues.append((gene, pearson_p, kruskal_p, trend))



result_txt = os.path.join(DESKTOP_PATH, "genes_results.txt")
with open(result_txt, 'w') as f:
    f.write("Gene\tPearson_p\tKruskal_p\tTrend\n")
    for gene, p1, p2, trend in results_with_pvalues:
        f.write(f"{gene}\t{p1:.4e}\t{p2:.4e}\t{trend}\n")
...
edgelist_path = os.path.join(na_folder, "edgelist_full.txt")
...
for i, age in enumerate(age_groups, start=1):
    ...
    file_path = os.path.join(na_folder, f"group{i}.txt")
    edgelist = generate_edgelist(group_data, selected_genes, file_path)
    group_edgelists[f"group{i}"] = edgelist
...
plot_network(edgelist_completa, "Network", os.path.join(na_folder, "network.png"))


for i, (group_name, edgelist_df) in enumerate(group_edgelists.items(), start=1):
    plot_network(edgelist_df, f"Network {group_name}", os.path.join(na_folder, f"network_{group_name}.png"))
...
def get_pathways_from_string(string_id):
    url = "https://string-db.org/api/json/annotation"
    params = {
        "identifiers": string_id,
        "species": 9606,
...
pathway_output_path = os.path.join(DESKTOP_PATH, "pathway_annotations.txt")


with open(pathway_output_path, 'w') as f:
    f.write("Gene\tMapped_Name\tPathway_Annotations\n")
    for gene in selected_genes:
        string_id, mapped_name = map_gene_to_string_id(gene)
        if string_id:
            pathways = get_pathways_from_string(string_id)
            if pathways:
                pathway_text = "; ".join([f"{pid}, {desc}" for pid, desc in pathways])
            else:
                pathway_text = "N/A"
        else:
            pathway_text = "ID non trovato"
        f.write(f"{gene}\t{mapped_name}\t{pathway_text}\n")
